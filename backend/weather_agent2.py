...
def webhook():
    try:
        ...
        prompt = f"What's the weather like in {location}? Give a brief, natural response with two recommendation."
        response = claude.invoke([HumanMessage(content=prompt)])

        response_payload = {
            "location": location,
            "analysis": response.content,
            "price": 1.99  # Including price in response
        }
...
def init_app():
    ...
    try:
        weather_identity = Identity.from_seed(os.getenv("WEATHER_KEY2"), 0)
        create_graph()
        ...
        register_with_agentverse(
            ...
            url="http://localhost:5008/webhook",  # Change port for each agent
            agentverse_token=os.getenv("AGENTVERSE_API_KEY"),
            agent_title="Premium Weather Assistant",
            readme=readme
        )
...
if __name__ == "__main__":
    load_dotenv()
    init_app()
    app.run(host="0.0.0.0", port=5008)  # Change port for each agent